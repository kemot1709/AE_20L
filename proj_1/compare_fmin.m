...
%     if flag_save
%         saveas(gcf,sprintf('wykres/Quasi_base_%i_fval',i));close;
%         saveas(gcf,sprintf('wykres/Quasi_base_%i_xy',i));close;
%     end
%         
...
%     plot_optimalization(history);
%     if flag_save
%         saveas(gcf,sprintf('wykres/Quasi_grad_%i_fval',i));close;
%         saveas(gcf,sprintf('wykres/Quasi_grad_%i_xy',i));close;
%     end
    ...
    plot_optimalization(history);
    if flag_save
        saveas(gcf,sprintf('wykres/Trust_grad_%i_fval',i));close;
        saveas(gcf,sprintf('wykres/Trust_grad_%i_xy',i));close;
    end
    
    % Trust-region with hessian
    [~,~,exitflag,output,~,~] = fminunc_trust_hess([x_start(i), y_start(i)],fun_grad);
    if exitflag ~= 1
    ...
    plot_optimalization(history);
    if flag_save
        saveas(gcf,sprintf('wykres/Trust_hess_%i_fval',i));close;
        saveas(gcf,sprintf('wykres/Trust_hess_%i_xy',i));close;
    end
end